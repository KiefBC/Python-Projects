...
indexes = []
counter = 0  # will use this to track the index #

# for i "items" in seq
for i in seq:
    if i == find:  # if {i} is equal to our {find}
        indexes.append(str(counter))  # append our {index} with the current {count} value
    counter += 1  # there might be further {find} values down the line
if len(indexes) == 0:  # if no {find} were found and appended to {indexes}
    print("not found")
else:  # print with a space in between, our {indexes}
    print(" ".join(indexes))
